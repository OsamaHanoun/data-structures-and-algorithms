...
const sortMeetingsByDay = (arr) => {
  ...
  arr.sort((a, b) => {
    ...
    let aPriority = priorityValue(a.dayOfWeek);
    let bPriority = priorityValue(b.dayOfWeek);
  ...
  arr.sort((a, b) => {
    if(a.dayOfWeek == b.dayOfWeek){
      if (Number(a.start) < Number(b.start)) return -1;
        ...
        if ((Number(a.end) - Number(a.start)) < (Number(b.end) - Number(b.start))) return -1;
        else if ((Number(a.end) - Number(a.start)) > (Number(b.end) - Number(b.start))) return 1;
        else return 0;
      }
    }
  });
